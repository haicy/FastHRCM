****************************************************************************                             
	                              MtHRCM
  (Multi-thread Parallel Compression for Large Collections of Genomes )

     https://github.com/haicy/MtHRCM
...
1.1 MtHRCM is implemented with Java and suggested to be run on Linux operating system.
1.2 BSC executable file should be put in the same directory of the executable jar package (e.g. MtHRCM.jar)
...
2.1 Usage
java -jar [-Xmx20g] MtHRCM.jar {file_name.txt} {compress | decompress} [thread_number]
   -[-Xms20g] is the initial allocation of memory, optianal. Please make sure your machine has more than 10GB physical memory.
   -MtHRCM.jar is the compression executable file,required.
...
3.2 compress and decompress hg17_chr22.fa and hg18_chr22.fa, using hg13_chr22.fa as reference. The paths of reference file and to-be-compressed files are written in chr22.txt in turn.
    java -jar -Xmx20g MtHRCM.jar chr22.txt compress 6
    output: chr22.bsc


    java -jar -Xmx20g MtHRCM.jar chr22.txt decompress
